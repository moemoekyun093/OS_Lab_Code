...
struct memory_manager{
	int freelist[PAGESIZE/MINALLOC];
	char* region;
} myman;

int init_alloc()
{
	myman.region=mmap(0,PAGESIZE,PROT_READ|PROT_WRITE|PROT_EXEC,MAP_PRIVATE|MAP_ANON,-1,0);
...
int cleanup()
{
	if(!munmap(myman.region,PAGESIZE))
		return 0;
	return -1;
}
...
char* alloc(int bytes)
{
	...
	{
		if(j==i-chunk+1)
			myman.freelist[j]=chunk;
		else
			myman.freelist[j]=1;
	}
	// printf("%p\n",myman.region);
	char *p=myman.region+MINALLOC*(i-chunk+1);
	// printf("%p\n",p);
	return p;
...
{
	int i;
	int a=(int)(addr-myman.region)/MINALLOC;
	int chunk=myman.freelist[a];
	for(i=a;i<a+chunk;i++)
	{
		myman.freelist[i]=0;
	}
