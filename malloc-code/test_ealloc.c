...
struct memory_manager{
	int freelist[MAX_NO][PAGESIZE/MINALLOC];
	char* region[MAX_NO];
} myman;
...
{
	if(bytes%MINALLOC != 0 || bytes<=0)
		return 0x0;
	int i;
	int flag=0;
  int flag2=0;
	int chunk=bytes/MINALLOC;
	int found=0;
  for(int k=0;k<MAX_NO;k++)
  {
    if(!myman.region[k])
      myman.region[k]=mmap(0,PAGESIZE,PROT_READ|PROT_WRITE|PROT_EXEC,MAP_PRIVATE|MAP_ANON,-1,0);
    for(i=0;i<PAGESIZE/MINALLOC;i++)
    {
      if(myman.freelist[k][i]==0)
        found++;
      if(found==chunk){
        flag=1;
        break;}
    ...
    {
      if(j==i-chunk+1)
        myman.freelist[k][j]=chunk;
      else
        myman.freelist[k][j]=1;
    }
    // printf("%p\n",myman.region);
	  char *p=myman.region[k]+MINALLOC*(i-chunk+1);
	  // printf("%p\n",p);
	  return p;
...
void dealloc(char* addr)
{
  ...
  for(k=0;k<MAX_NO;k++)
  {
   int a=(int)(addr-myman.region[k]);
   if(a>=0 && a<PAGESIZE)
    break;
  }
  int i;
	int a=(int)(addr-myman.region[k])/MINALLOC;
	int chunk=myman.freelist[k][a];
	for(i=a;i<a+chunk;i++)
	{
		myman.freelist[k][i]=0;
	}
	return;
}
